<div class="items-center bg-gray-50 rounded-lg shadow sm:flex dark:bg-gray-800 dark:border-gray-700 min-w-2xl">
    <img class="max-w-full rounded-lg sm:rounded-none sm:rounded-l-lg"
        src="https://flowbite.s3.amazonaws.com/blocks/marketing-ui/avatars/bonnie-green.png" alt="Bonnie Avatar">
    <div class="p-5 text-nowrap">
        <h3 class="text-xl font-bold tracking-tight text-gray-900 dark:text-white">
            Bonnie Green
        </h3>
        <span class="text-gray-500 dark:text-gray-400">CEO & Web Developer</span>
    </div>
</div>
